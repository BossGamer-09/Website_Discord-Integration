"""
app/pilotboard/models.py

Pilot roster management system.
Tracks pilots' skill ratings (team fighting, duelling, leadership),
...
# ---------------------------------------------------------------------------
# Tag constants — mirrors the frontend TagBadge definitions
# ---------------------------------------------------------------------------

class PilotTag(models.TextChoices):
    COMP_TEAM    = "comp_team",    "Comp Team"
...
class Pilot(models.Model):
    """A tracked org pilot with skill ratings, tags, notes, and goals."""
    ...
    # Stored as a JSON list of PilotTag keys, e.g. ["comp_team", "trial"]
    ...
    def save(self, *args, **kwargs):
        if not self.slug or kwargs.get("update_fields") and "name" in kwargs["update_fields"]:
            base = slugify(self.name) or "pilot"
            slug, n = base, 2
            while Pilot.objects.filter(slug=slug).exclude(pk=self.pk).exists():
                slug = f"{base}-{n}"
                n += 1
            self.slug = slug
    ...
    def skill_total(self):
        return self.team_fighting + self.duelling + self.leadership
...
class PilotGoal(models.Model):
    """A training / improvement goal attached to a pilot. One open at a time."""

    pilot     = models.ForeignKey(Pilot, on_delete=models.CASCADE, related_name="goals")
    ...
    def __str__(self):
        ...
        return f"[{status}] {self.pilot.name}: {self.title}"
...
# ---------------------------------------------------------------------------
# PilotGroupFilter — configures which Django Groups appear as roster filters
# ---------------------------------------------------------------------------

class PilotGroupFilter(models.Model):
...
class GoalNote(models.Model):
    ...
    goal      = models.ForeignKey(PilotGoal, on_delete=models.CASCADE, related_name="notes")
